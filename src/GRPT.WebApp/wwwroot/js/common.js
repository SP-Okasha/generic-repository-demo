...
class FlexyForms {
    ...
    static SetMessage(inputId, message, messageType) {
        ...
        switch (messageType) {
            case 'success':
                html = `<span class="text-success"><i class="fas fa-check-circle"></i> ${message}</span>`;
                inputBorder = 'border-success'
                break;
            case 'info':
            ...
            default:
                ...
                inputBorder = message ? 'border-danger' : ''
        }

        $(inputId).addClass(inputBorder)
    ...
    //Parameter >> Array of properties (DataListUrl, ControlId)
    static LoadDataLists(controlProperties) {
        $.each(controlProperties, function (index, control) {
            $.get(control.DataListUrl, function (response) {
                $(control.ControlId).empty();
                $.each(response.data, function (index, item) {
                    $(control.ControlId).append(`<option value="${item.id}">${item.name}</option>`);
                })
            });
        })
    }
...
class FlexyUtil {
    ...
    static MessageBox(message, settings, callback, callback_2) {
        ...
        var modalHtml = "<div id='" + globalSettings.modalId.replace('#', '') + "' class='modal customized shadow' style='z-index:9990' role='dialogue'>"
            ...
            + "<div class='modal-header justify-content-center bg-theme text-white'>"
            + "" + icon + ""
            + "</div>"
